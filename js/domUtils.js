...
export function updateNationList() {
  ...
  if (count === 0) {
    ...
    cfg.nations.forEach((nation, index) => {
      ...
      const coordX =
        nation?.coordinates?.[0] !== undefined
          ? Math.round(nation.coordinates[0])
          : '?';
      const coordY =
        nation?.coordinates?.[1] !== undefined
          ? Math.round(nation.coordinates[1])
          : '?';
      ...
      li.addEventListener('click', (event) => {
        ...
        if (
          isNaN(targetIndex) ||
          targetIndex < 0 ||
          targetIndex >= cfg.nations.length
        )
          return;
      ...
      li.addEventListener('dblclick', (event) => {
        ...
        if (
          ...
          targetIndex < 0 ||
          targetIndex >= cfg.nations.length ||
          !cfg.nations[targetIndex]?.coordinates
        )
          return;
...
export function updateInfoPanel(nationIndex) {
  const requiredElements = [
    cfg.infoNameSpan,
    cfg.infoStrengthSpan,
    cfg.infoPlaceholder,
    cfg.infoFlagPreview,
    cfg.infoFlagStatus,
    cfg.infoFlagUploadLabel,
    cfg.infoFlagUploadInput,
    cfg.infoFlagRemoveButton,
    cfg.editFlagButton,
  ];
  ...
  const noNationSelected =
    nationIndex === null ||
    nationIndex < 0 ||
    nationIndex >= cfg.nations.length;
  ...
  if (noNationSelected) {
    ...
    if (nation) {
      ...
      if (hasDisplayableFlag) {
        ...
        const originalFileName = nation.flag
          ? `${nation.flag}.${nation.flagDataType || '?'}`
        ...
        const assumedExt = nation.flag.toLowerCase().endsWith('.svg')
          ? 'svg'
          : nation.flag.toLowerCase().endsWith('.png')
            ? 'png'
            : '...';
      ...
      console.error(
        `Could not find nation data for valid index: ${nationIndex}`
      );
...
export function openInlineEditor(index, mapX, mapY) {
  if (
    !cfg.inlineEditPanel ||
    !cfg.inlineEditName ||
    !cfg.inlineEditStrength ||
    index < 0 ||
    index >= cfg.nations.length
  )
    return;
  ...
  if (panelX + panelWidth > containerWidth - 10) {
    panelX = canvasPos.x - panelWidth - markerScreenRadius - panelMargin;
  }
...
export function applySettings() {
  if (
    !cfg.markerSizeInput ||
    !cfg.nationTextSizeInput ||
    !cfg.flagSizeInput ||
    !cfg.darkModeToggle ||
    !cfg.markerSizeValue ||
    !cfg.nationTextSizeValue ||
    !cfg.flagSizeValue
  ) {
    console.warn(
      'Cannot apply settings: One or more settings elements not found.'
    );
    return;
  }
...
export function saveSettings() {
  if (
    ...
    console.warn(
      'Cannot save settings: One or more settings elements not found.'
    );
    return;
  }
  try {
    localStorage.setItem(
      'mapEditor_markerRadius',
      cfg.markerRadius().toString()
    );
    localStorage.setItem(
      'mapEditor_nationTextSize',
      cfg.nationTextSize().toString()
    );
    localStorage.setItem(
      'mapEditor_flagBaseDisplaySize',
      cfg.flagBaseDisplaySize().toString()
    );
    localStorage.setItem(
      'mapEditor_darkMode',
      cfg.darkModeToggle.checked.toString()
    );
  } catch (e) {
...
export function loadSettings() {
  if (
    ...
    console.warn(
      'Cannot load settings: One or more settings elements not found.'
    );
    applySettings();
    return;
  }
  try {
    ...
    if (savedRadius !== null) {
      ...
      if (
        !isNaN(radius) &&
        cfg.markerSizeInput &&
        radius >= parseInt(cfg.markerSizeInput.min, 10) &&
        radius <= parseInt(cfg.markerSizeInput.max, 10)
      ) {
    ...
    if (savedTextSize !== null) {
      ...
      if (
        !isNaN(size) &&
        cfg.nationTextSizeInput &&
        size >= parseInt(cfg.nationTextSizeInput.min, 10) &&
        size <= parseInt(cfg.nationTextSizeInput.max, 10)
      ) {
    ...
    if (savedFlagSize !== null) {
      ...
      if (
        !isNaN(size) &&
        cfg.flagSizeInput &&
        size >= parseInt(cfg.flagSizeInput.min, 10) &&
        size <= parseInt(cfg.flagSizeInput.max, 10)
      ) {
...
export function showModal(type, title, message, options = {}) {
  return new Promise((resolve) => {
    ...
    if (cfg.currentModalResolve) {
      console.warn(
        'Modal system busy. Cannot show new modal until previous one is closed.'
      );
      return resolve(null);
    }
    ...
    cfg.modalInputContainer.style.display = 'none';
    ...
    cfg.modalDialog.onkeydown = null;
    ...
    switch (type) {
      ...
      case 'confirm':
        ...
        cfg.modalConfirm.onclick = () => {
          cfg.currentModalResolve?.(true);
          hideModal();
        };
        cfg.modalDeny.onclick = () => {
          cfg.currentModalResolve?.(false);
          hideModal();
        };
      ...
      case 'prompt':
        ...
        cfg.modalOk.onclick = () => {
          cfg.currentModalResolve?.(cfg.modalInput.value);
          hideModal();
        };
        cfg.modalCancel.onclick = () => {
          cfg.currentModalResolve?.(null);
          hideModal();
        };
        primaryButton = cfg.modalInput;
    ...
    if (primaryButton) {
      setTimeout(() => {
        if (document.body.contains(primaryButton)) {
          primaryButton.focus();
          if (type === 'prompt') primaryButton.select();
        }
      }, 0);
    }
  });
