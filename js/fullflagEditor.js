...
const FFE_OPTIONS = {
  ...
  RATIOS: {
    ...
    '8:5': { width: 160, height: 100 },
    '5:3': { width: 167, height: 100 },
    '16:9': { width: 178, height: 100 },
...
export function initFullFlagEditor() {
  ...
  if (!ffeContainer) {
    console.error(
      'Full Flag Editor container (#full-flag-editor-view) not found!'
    );
    return;
  }
            ...
            <div class="ffe-options-panel">
                 ...
                 <div class="ffe-option-group ffe-settings-group">
                     ...
                     <select id="ffeRatioSelect" class="select">
                         ${Object.entries(FFE_OPTIONS.RATIOS)
                           .map(
                             ([key, value]) =>
  ...
  ffeDetailOriginalPreview = ffeContainer.querySelector(
    '#ffeDetailOriginalPreview'
  );
  ffeDetailStandardizedPreview = ffeContainer.querySelector(
    '#ffeDetailStandardizedPreview'
  );
  ffeDetailOriginalTitle = ffeContainer.querySelector(
    '#ffeDetailOriginalTitle'
  );
  ffeDetailStandardizedTitle = ffeContainer.querySelector(
    '#ffeDetailStandardizedTitle'
  );
  ...
  ffePreprocessModeSelect = ffeContainer.querySelector(
    '#ffePreprocessModeSelect'
  );
  ffeRatioSelect = ffeContainer.querySelector('#ffeRatioSelect');
  ffeFinalScaleModeSelect = ffeContainer.querySelector(
    '#ffeFinalScaleModeSelect'
  );
...
function setFfeStatus(message, isError = false) {
  ...
  ffeStatus.style.color = isError
    ? 'var(--status-error-color, red)'
    : 'var(--text-color)';
...
function updateDetailPreviews() {
  ...
  if (flag) {
    ...
    ffeDetailOriginalPreview.alt = flag.originalFileName
      ? `Original: ${flag.originalFileName}`
      : 'Original';
    ffeDetailOriginalTitle.textContent = flag.originalFileName
      ? `Original (${flag.originalWidth}x${flag.originalHeight})`
      : 'Original';
    ...
    if (flag.standardizedUrl && flag.status === 'ready') {
      ...
      ffeDetailStandardizedPreview.src = '';
      ffeDetailStandardizedPreview.alt =
        flag.status === 'loading' ? 'Loading...' : 'Processing...';
...
function getCurrentFfeOptions() {
  ...
  const dims =
    FFE_OPTIONS.RATIOS[ratio] || FFE_OPTIONS.RATIOS[FFE_OPTIONS.DEFAULT_RATIO];

  return {
    preprocessMode:
      ffePreprocessModeSelect?.value || FFE_OPTIONS.DEFAULT_PREPROCESS,
...
function updateBatchActionButtons() {
  const selectedCount = processedFlags.filter((flag) => flag.isSelected).length;
  const readyCount = processedFlags.filter(
    (flag) => flag.status === 'ready'
  ).length;
  const selectedReadyCount = processedFlags.filter(
    (flag) => flag.isSelected && flag.status === 'ready'
  ).length;
...
async function handleFfeApplySettings() {
  ...
  const promises = selectedFlags.map((flag) =>
    (async () => {
      ...
      try {
        ...
        console.error(
          `Error applying settings to ${flag.originalFileName}:`,
          error
        );
        flag.status = 'error';
        flag.errorMessage = error.message;
  ...
  setFfeStatus(
    `Finished applying settings to ${selectedFlags.length} flag(s). Check list for errors.`
  );
...
async function saveFlagsAsZip(
  flagsToSave,
  zipFilename = 'standardized_flags.zip'
) {
  const readyFlags = flagsToSave.filter(
    (f) => f.status === 'ready' && f.standardizedUrl
  );
  ...
  try {
    ...
    await Promise.all(
      readyFlags.map(async (flag) => {
        try {
          const baseName =
            flag.originalFileName.split('.').slice(0, -1).join('.') || flag.id;
          const svgFilename = `${baseName}.svg`;

          let stdWidth = flag.originalWidth;
          ...
          try {
            ...
            console.warn(
              `Could not load standardized image ${flag.originalFileName} for ZIP dims. Using defaults/approximations. Error:`,
              loadErr
            );
    ...
    if (addedCount === 0) {
      setFfeStatus('No flags could be prepared for saving.', true);
      return;
    }
    ...
    setFfeStatus(
      `Saved ${addedCount} flags to ${zipFilename}.` +
...
function handleFfeExportClick() {
  const selectedReadyFlags = processedFlags.filter(
    (f) => f.isSelected && f.status === 'ready'
  );
  ...
  if (
    ...
    try {
      ...
      try {
        ...
        console.warn(
          `Could not load std image for single save: ${flag.originalFileName}. Using approx dims.`,
          loadErr
        );
        const currentOpts = getCurrentFfeOptions();
        stdWidth = currentOpts.finalWidth;
      ...
      console.error(
        `Error saving single flag ${flag.originalFileName}:`,
        error
      );
      setFfeStatus(
        `Error saving ${flag.originalFileName}: ${error.message}`,
        true
      );
    }
