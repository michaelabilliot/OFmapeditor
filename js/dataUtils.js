...
export async function processAndAssignFlag(file, nation) {
  return new Promise((resolve, reject) => {
    if (!file || !nation) {
      return reject(new Error('Invalid arguments for processAndAssignFlag'));
    }
    ...
    let isSvg =
      fileMimeType === 'image/svg+xml' || (!fileMimeType && fileExt === 'svg');
    ...
    if (!isSvg && !isRasterImage) {
      console.warn(
        `File ${file.name} has unsupported type: ${fileMimeType || fileExt || 'Unknown'}. Skipping.`
      );
      return reject(
        new Error(
          `Unsupported file type for ${file.name}: ${fileMimeType || 'Unknown'}`
        )
      );
    }
    ...
    reader.onload = (e) => {
      try {
        ...
        nation.flagDataType = isSvg
          ? 'svg'
          : fileMimeType?.split('/')[1] || fileExt || 'png'; // Store original type ('png', 'jpeg', 'svg', etc.)

        // --- 2. Create an Image object from the original data to get dimensions ---
        const originalFlagImage = new Image();

        originalFlagImage.onload = async () => {
          ...
          if (nation.flagWidth === 0 || nation.flagHeight === 0) {
            console.warn(
              `Loaded flag for ${nation.name} but natural dimensions are zero. Check file: ${file.name}`
            );
            // Cannot standardize or use effectively, clear drawing image but keep original data
            nation.flagImage = null;
            return resolve({
              status: 'loaded_zero_dims',
              filename: file.name,
          ...
          try {
            ...
            finalStandardizedImage.onload = () => {
              ...
              console.log(
                `Flag automatically standardized for drawing: ${nation.name} (${flagName}.${nation.flagDataType}) | Original: ${nation.flagWidth}x${nation.flagHeight}`
              );
              // Resolve the main promise once the standardized image is loaded
              resolve({
                status: 'loaded_standardized',
                filename: file.name,
                nationName: nation.name,
              });
            };
            finalStandardizedImage.onerror = (err) => {
              console.error(
                `Error loading STANDARDIZED flag bitmap into final Image object for: ${nation.name} from ${file.name}`,
                err
              );
              nation.flagImage = null; // Clear potentially broken image ref
              reject(
                new Error(
                  `Failed to load standardized flag bitmap data for ${file.name}`
                )
              );
            };
            ...
            console.error(
              `Automatic flag standardization failed for ${nation.name} (${file.name}):`,
            ...
            reject(
              new Error(
                `Standardization failed for ${file.name}: ${standardizationError.message}`
              )
            );
          }
        ...
        originalFlagImage.onerror = (err) => {
          console.error(
            `Error loading ORIGINAL flag image into Image object for: ${nation.name} from ${file.name}`,
            err
          );
          ...
          reject(
            new Error(
              `Failed to load original flag image data for ${file.name}`
            )
          );
        };
        ...
        if (isSvg) {
          try {
            ...
            console.error(
              `Error preparing SVG for initial load ${nation.name}:`,
              svgError
            );
            reject(
              new Error(
                `Failed to prepare SVG data for ${file.name}: ${svgError.message}`
              )
            );
          }
        } else {
          // PNG, JPG, GIF, WEBP etc. (already read as DataURL)
          originalFlagImage.src = originalFileData;
        }
      } catch (loadError) {
        console.error(
          `Error processing file data for ${nation.name} (${file.name}):`,
          loadError
        );
        ...
        nation.flagHeight = null;
        reject(
          new Error(`Processing error for ${file.name}: ${loadError.message}`)
        );
      }
    }; // End reader.onload
...
export async function loadFlagFiles(files) {
  ...
  if (cfg.nations.length === 0) {
    await showModal(
      'alert',
      'Info',
      'No nations loaded yet. Load JSON or add nations first.'
    );
    return;
  }
  ...
  if (!jsonSpecifiedFlags && nationGeneratedNameMap.size === 0) {
    await showModal(
      'alert',
      'Info',
      'No nations require flags (none specified in JSON, none could be generated).'
    );
  ...
  const fileLoadPromises = Array.from(files).map((file) => {
    return (async () => {
      // Use an async IIFE to handle awaits within the map
      try {
        ...
        if (nationFlagMap.has(potentialMatchNameLower)) {
          ...
          if (
            matchedNation.flag &&
            matchedNation.flag.toLowerCase() !== potentialMatchNameLower
          ) {
            console.log(
              `Flag file '${file.name}' matches generated name for '${matchedNation.name}', but JSON specified '${matchedNation.flag}'. Using this file anyway.`
            );
          }
        }
        ...
        if (!matchedNation) {
          ...
          if (
            !nationFlagMap.has(potentialMatchNameLower) &&
            !nationGeneratedNameMap.has(potentialMatchNameLower)
          ) {
            skippedNoMatchCount++;
          }
        ...
        if (!isSvg && !isRasterImage) {
          console.warn(
            `Skipping file with unsupported type: ${file.name} (Type: ${fileMimeType || fileExt || 'unknown'})`
          );
          skippedTypeCount++;
        ...
        try {
          ...
          if (
            result.status === 'loaded_standardized' ||
            result.status === 'loaded_zero_dims'
          ) {
          ...
          console.error(
            `Error processing flag file ${file.name} for ${matchedNation.name}:`,
            processingError
          );
          errorCount++;
          return {
        ...
        console.error(
          `Unexpected error setting up processing for ${file.name}:`,
          err
        );
        errorCount++;
  ...
  let isError = false;
  if (errorCount > 0) {
    statusParts.push(`Errors: ${errorCount}.`);
    isError = true;
  }
  if (skippedNoMatchCount > 0)
    statusParts.push(`Skipped (no match): ${skippedNoMatchCount}.`);
  if (skippedTypeCount > 0)
    statusParts.push(`Skipped (bad type): ${skippedTypeCount}.`);
  ...
  if (jsonSpecifiedFlags) {
    ...
    if (missingFromJson.length > 0) {
      statusParts.push(
        `Missing/failed for JSON specified flags: ${missingFromJson.join(', ')}.`
      );
...
export function promptAndLoadFlags() {
  ...
  if (cfg.nations.length === 0) {
    showModal(
      'alert',
      'Info',
      'Add nations or load JSON before loading flags.'
    );
    return;
  }
...
export async function handleJsonLoad(file) {
  ...
  if (!cfg.mapImage) {
    await showModal(
      'alert',
      'Error',
      'Load map image BEFORE loading JSON data.'
    );
    return;
  }
  if (!cfg.mapInfo || cfg.mapInfo.width === 0 || cfg.mapInfo.height === 0) {
    await showModal(
      'alert',
      'Error',
      'Map dimensions not loaded correctly. Reload map image.'
    );
    return;
  }
  ...
  reader.onload = async (e) => {
    ...
    try {
      ...
      if (
        ...
        throw new Error(
          "Invalid JSON: Missing or invalid numeric 'width'/'height' > 0."
        );
      if (!Array.isArray(loadedData.nations))
        throw new Error("Invalid JSON: Missing or invalid 'nations' array.");
      ...
      if (
        jsonWidth !== cfg.mapInfo.width ||
        jsonHeight !== cfg.mapInfo.height
      ) {
        ...
        const confirmScale = await showModal(
          'confirm',
          'Dimension Mismatch',
          confirmMsg,
          { confirmText: 'Scale', denyText: 'Cancel Load' }
        );

        if (!confirmScale) {
          // User chose Cancel Load
        ...
        console.log(
          `Scaling coordinates: X factor=${scaleX.toFixed(4)}, Y factor=${scaleY.toFixed(4)}`
        );
      }
      ...
      for (const [index, nation] of loadedData.nations.entries()) {
        ...
        if (typeof nation !== 'object' || nation === null)
          throw new Error(
            `Invalid data for ${nationIdentifier}: Not an object.`
          );
        if (
          !Array.isArray(nation.coordinates) ||
          nation.coordinates.length !== 2 ||
          typeof nation.coordinates[0] !== 'number' ||
          typeof nation.coordinates[1] !== 'number'
        )
          throw new Error(`Invalid coords for ${nationIdentifier}.`);
        if (typeof nation.name !== 'string' || nation.name.trim() === '')
          throw new Error(`Invalid or empty name for ${nationIdentifier}.`);
        if (
          typeof nation.strength !== 'number' ||
          !Number.isInteger(nation.strength)
        )
          throw new Error(
            `Invalid strength (must be integer) for ${nationIdentifier}.`
          );
        // Allow flag to be missing, null, or a non-empty string
        if (
          nation.hasOwnProperty('flag') &&
          nation.flag !== null &&
          (typeof nation.flag !== 'string' || nation.flag.trim() === '')
        )
          throw new Error(
            `Invalid 'flag' property (must be non-empty string or null) for ${nationIdentifier}.`
          );
        ...
        if (origX < 0 || origX > jsonWidth || origY < 0 || origY > jsonHeight) {
          console.warn(
            `${nationIdentifier} original coords [${origX}, ${origY}] outside JSON bounds (${jsonWidth}x${jsonHeight}).`
          );
          boundsWarnings++;
        }
        ...
        if (
          finalX < 0 ||
          finalX > cfg.mapInfo.width ||
          finalY < 0 ||
          finalY > cfg.mapInfo.height
        ) {
          console.warn(
            `${nationIdentifier} ${needsScaling ? 'scaled' : ''} coords [${finalX}, ${finalY}] are outside current map bounds (${cfg.mapInfo.width}x${cfg.mapInfo.height}).`
          );
          boundsWarnings++;
        }
      ...
      if (hasFlagsInJson) {
        const loadFlagsNow = await showModal(
          'confirm',
          'Flags Found',
          'The loaded JSON references nation flags.\nWould you like to select the corresponding flag image files (PNG/SVG etc.) now?',
          { confirmText: 'Select Files', denyText: 'Later' }
        );
        if (loadFlagsNow) {
          promptAndLoadFlags(); // Trigger the flag loading dialog
        } else {
          updateStatus(
            statusMessage + " Use 'Load Flags' button later if needed."
          );
        }
      }
    } catch (error) {
...
export async function saveProjectAsZip() {
  ...
  if (!cfg.mapImage || !cfg.mapInfo.fileName) {
    await showModal(
      'alert',
      'Error',
      'Load and process a map image before saving.'
    );
    return;
  }
  if (!cfg.mapInfo.width || !cfg.mapInfo.height) {
    await showModal(
      'alert',
      'Error',
      'Map dimensions missing. Reload map image.'
    );
    return;
  }
  if (cfg.nations.length === 0) {
    const confirmEmpty = await showModal(
      'confirm',
      'Warning',
      'No nations have been added. Save project with just the map image?',
      { confirmText: 'Save Map Only', denyText: 'Cancel' }
    );
  ...
  const defaultProjectName =
    cfg.mapInfo.name !== 'Untitled Map'
      ? cfg.mapInfo.name
      : cfg.mapInfo.fileName
        ? cfg.mapInfo.fileName.split('.').slice(0, -1).join('.')
        : 'MyMapProject';
  const projectNameInput = await showModal(
    'prompt',
    'Save Project',
    'Enter project name (used for map image/json filenames):',
    { defaultValue: defaultProjectName }
  );
  if (projectNameInput === null) return; // User cancelled
  const safeProjectName =
    projectNameInput.trim().replace(/[^a-z0-9_-]/gi, '_') || 'map_project'; // Sanitize name

  // --- Get ZIP Filename ---
  const defaultZipFileName = `${safeProjectName}.zip`;
  const outputZipFilenameInput = await showModal(
    'prompt',
    'Save Project',
    'Enter filename for the ZIP file:',
    { defaultValue: defaultZipFileName }
  );
  ...
  if (!finalZipFilename || finalZipFilename === '.zip') {
    await showModal('alert', 'Error', 'Invalid ZIP filename provided.');
    return;
  }
  ...
  try {
    ...
    if (!projectFolder)
      throw new Error('Could not create project folder in ZIP.'); // Check if folder creation worked
    ...
    const mapData = {
      ...
      nations: cfg.nations.map((n) => {
        ...
        const nationData = {
          coordinates: [
            Math.round(n.coordinates[0]),
            Math.round(n.coordinates[1]),
          ],
          name: n.name,
          strength: n.strength,
        };
    ...
    let finalJsonString = jsonString.replace(
      coordRegex,
      (match, p1, p2) => `"coordinates": [${p1}, ${p2}]`
    ); // Add space back
    ...
    if (mapImageBase64) {
      ...
      const extension = cfg.mapInfo.fileType?.startsWith('image/')
        ...
        : 'png';
      projectFolder.file(`${safeProjectName}.${extension}`, mapImageBase64, {
        base64: true,
      });
    } else {
      ...
      await showModal(
        'alert',
        'Warning',
        'Could not include the map image data in the ZIP.'
      );
    }
    ...
    const flagPromises = cfg.nations.map(async (nation) => {
      ...
      if (
        ...
        try {
          ...
          if (
            standardizedDataUrl &&
            standardizedDataUrl.startsWith('data:image') &&
            standardizedWidth > 0 &&
            standardizedHeight > 0
          ) {
            ...
            console.warn(
              `Could not get valid standardized data/dimensions for flag '${nation.name}' (file: ${flagFileName}). Skipping save.`
            );
            flagSaveErrors++;
          }
        } catch (saveError) {
          console.error(
            `Error preparing standardized flag '${nation.name}' (file: ${flagFileName}) for saving:`,
            saveError
          );
          flagSaveErrors++;
        }
        ...
        console.warn(
          `Could not save flag for '${nation.name}' (specified as '${nation.flag}'): Standardized flag image data not available or invalid.`
        );
    ...
    console.log(
      `Attempted to save standardized flags. Added: ${flagsAddedCount}, Errors/Skipped: ${flagSaveErrors}.`
    );
    ...
    if (flagSaveErrors > 0)
      finalStatus += ` (${flagSaveErrors} flag save error(s) - see console).`;
    updateStatus(finalStatus);
  } catch (error) {
    console.error('Error creating ZIP file:', error);
    const errorMsg =
      error instanceof Error ? error.message : 'Unknown error creating ZIP.';
    await showModal(
      'alert',
      'ZIP Error',
      `Error creating ZIP file: ${errorMsg}`
    );
