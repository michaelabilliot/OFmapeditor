...
import { updateStatus, showModal, updateNationList, updateInfoPanel, closeInlineEditor, toggleColorizerControls } from './domUtils.js';
    ...
    return new Promise((resolve, reject) => {
        ...
        try {
            ...
            for (let i = 0; i < data.length; i += 4) {
                ...
                if (a < 20 || (r === 0 && g === 0 && b === 0) || (b >= 104 && b <= 108)) {
                    newR = waterColor.r;
                    newG = waterColor.g;
                    newB = waterColor.b;
                } else {
                    ...
                    if (intensity <= lowThreshold) {
                        ...
                        const amount = (intensity - lowThreshold) / (midThreshold - lowThreshold);
                        finalColor = lerpColor(plainsColor, highlandsColor, amount);
                    } else if (intensity <= highThreshold) {
                        const amount = (intensity - midThreshold) / (highThreshold - midThreshold);
                        finalColor = lerpColor(highlandsColor, mountainsColor, amount);
                    } else { // intensity > highThreshold
                        const amount = Math.min(1, (intensity - highThreshold) / (255 - highThreshold)); // Clamp amount
...
export async function handleMapImageLoad(file) {
    ...
    if (!file.type.startsWith('image/')) {
        await showModal('alert', 'Invalid File Type', `Selected file (${file.name}) does not appear to be an image.`);
    ...
    reader.onload = async (e) => {
        ...
        tempImage.onload = async () => {
            ...
            try {
                ...
                const colorizedDataUrl = await colorizeLoadedMap(cfg.originalMapImage, file.type, defaultThresholds);
                ...
                displayImage.onload = () => {
                    ...
                    showModal('alert', 'Adjust Colors', 'The map has been colorized. Use the sliders at the top to adjust the terrain zones, then click "Confirm Colors" to start editing. WARNING: MAPS THAT ARE BIG *WILL* LAG. ');
                ...
                await showModal('alert', 'Map Processing Error', `Failed to colorize the map: ${errorMessage}`);
        ...
        tempImage.onerror = async () => {
            await showModal('alert', 'Image Load Error', `Error loading the selected image file (${file.name}).`);
...
export function finalizeMapLoad() {
    ...
    updateStatus(`Map ready: ${cfg.mapInfo.fileName} (${cfg.mapInfo.width}x${cfg.mapInfo.height}).`);
