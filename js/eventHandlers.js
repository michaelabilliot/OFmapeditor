...
export async function handleMapLoadClick(event) {
  ...
  if (!cfg.imageInput) {
    ...
    domUtils.showModal(
      'alert',
      'Error',
      'Cannot find map image input element. Check HTML ID.'
    );
    return;
  }
...
export async function handleJsonFileSelect(event) {
  if (!cfg.mapImage) {
    await domUtils.showModal(
      'alert',
      'Error',
      'Load a map image BEFORE loading JSON data.'
    );
...
export function handleCanvasMouseDown(event) {
  ...
  if (
    cfg.inlineEditPanel.style.display === 'block' &&
    !isClickOnEditor &&
    clickedNationIndex !== cfg.nationIndexBeingEdited
  ) {
  ...
  if (clickedNationIndex !== null) {
    ...
    domUtils.updateStatus(
      `Dragging ${cfg.nations[clickedNationIndex].name}...`
    );
...
export function handleCanvasMouseMove(event) {
  ...
  if (cfg.currentModalResolve || cfg.isPanningAnimationActive || !cfg.mapImage)
    return;
  ...
  // --- Handle Dragging Nation ---
    ...
    if (!nation) {
      // Safety check if nation disappears mid-drag
      cfg.setDraggingNation(false);
      needsCursorUpdate = true;
...
export async function handleCanvasMouseUp(event) {
  ...
  if (cfg.potentialPan) {
    ...
    if (canvasPos) {
      ...
      if (Math.hypot(dx, dy) <= cfg.panThreshold) {
        ...
        const mapPosAdd = canvasUtils.canvasToMapCoords(
          cfg.mouseDownPos.x,
          cfg.mouseDownPos.y
        );
        if (mapPosAdd) {
          ...
          domUtils.updateStatus(
            'Could not determine map position for adding.',
            true
          );
        }
      } else {
  ...
  if (cfg.draggingNation) {
    ...
    domUtils.updateCursor();
    if (
      cfg.selectedNationIndex !== null &&
      cfg.nations[cfg.selectedNationIndex]
    ) {
      const nation = cfg.nations[cfg.selectedNationIndex];
...
export function handleCanvasWheel(event) {
  ...
  const delta = Math.max(
    -1,
    Math.min(1, -event.deltaY || -event.detail || event.wheelDelta)
  );
  ...
  const mapPosBeforeZoom = canvasUtils.canvasToMapCoords(
    canvasPos.x,
    canvasPos.y
  );
...
export async function handleDocumentKeyDown(event) {
  ...
  const isInlineEditorInputFocused =
    cfg.inlineEditPanel?.style.display === 'block' &&
    (activeElement === cfg.inlineEditName ||
      activeElement === cfg.inlineEditStrength);
  ...
  const isFlagEditorOpen =
    cfg.flagEditorModalContainer?.style.display === 'flex'; // Check container visibility
  ...
  switch (event.key) {
    ...
    case 'S':
      if (!event.ctrlKey && !event.metaKey) {
        // Avoid conflict with browser save
        event.preventDefault();
        if (cfg.saveButton && !cfg.saveButton.disabled) {
          // Check if save is enabled
          await dataUtils.saveProjectAsZip();
        } else {
          domUtils.updateStatus(
            'Save unavailable (load map/add nations first).'
          );
        }
      }
      break;
...
export async function handleFlagUploadChange(event) {
  ...
  if (
    cfg.selectedNationIndex === null ||
    cfg.selectedNationIndex < 0 ||
    cfg.selectedNationIndex >= cfg.nations.length
  ) {
    await domUtils.showModal(
      'alert',
      'Error',
      'Select a nation before uploading a flag.'
    );
    event.target.value = ''; // Clear input
    return;
  }
  ...
  if (!isSvg && !isRasterImage) {
    await domUtils.showModal(
      'alert',
      'Error',
      'Invalid file type. Please select PNG, SVG, JPG, GIF, or WEBP.'
    );
    event.target.value = '';
    return;
  }
  ...
  domUtils.updateStatus(
    `Processing & Standardizing flag ${file.name} for ${nation.name}...`
  );
  try {
    ...
    const errorMsg =
      error instanceof Error ? error.message : 'Unknown flag processing error.';
    await domUtils.showModal(
      'alert',
      'Flag Processing Error',
      `Could not process flag: ${errorMsg}`
    );
    domUtils.updateStatus(
      `Error processing flag ${file.name}: ${errorMsg}`,
      true
    );
...
export async function handleFlagRemoveClick() {
  ...
  if (!nation || (!nation.flag && !nation.flagImage && !nation.flagData)) {
    // Check flagData too
    domUtils.updateStatus('No flag to remove for selected nation.');
    return;
  }

  const confirmRemove = await domUtils.showModal(
    'confirm',
    'Remove Flag',
    `Remove flag from "${nation.name}"?`,
    { confirmText: 'Remove', denyText: 'Cancel' }
  );
...
export async function handleFlagEditorClick() {
  ...
  if (
    ...
    await domUtils.showModal(
      'alert',
      'Cannot Edit Flag',
      'No original flag data found for the selected nation. Upload a flag first.'
    );
    return;
  }
  ...
  try {
    ...
    if (manuallyStandardizedDataUrl) {
      ...
      domUtils.updateStatus(
        `Applying manually edited flag for ${nation.name}...`
      );
      ...
      manuallyEditedImage.onload = () => {
        ...
        domUtils.updateStatus(
          `Manually edited flag applied for ${nation.name}.`
        );
      ...
      manuallyEditedImage.onerror = async () => {
        console.error(
          `Failed to load the manually edited flag Data URL into an Image object for ${nation.name}.`
        );
        await domUtils.showModal(
          'alert',
          'Error',
          'Failed to apply the edited flag.'
        );
        domUtils.updateStatus(
          `Error applying edited flag for ${nation.name}.`,
          true
        );
    ...
    domUtils.updateStatus(
      `Error opening/using flag editor: ${error.message}`,
      true
    );
  }
}
...
export async function handleColorizerSliderChange() {
  ...
  try {
    ...
    const colorizedDataUrl = await mapUtils.colorizeLoadedMap(
      cfg.originalMapImage,
      cfg.mapInfo.fileType,
      thresholds
    );
    ...
    const errorMsg =
      error instanceof Error ? error.message : 'Unknown colorization error.';
