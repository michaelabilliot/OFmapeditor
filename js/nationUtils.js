...
export async function handleAddNation(mapPos) {
  ...
  const nameInput = await showModal(
    ...
    'Add Nation',
    'Enter Nation Name:',
    { placeholder: 'E.g., Kingdom of Example' }
  );
  ...
  let strength = null;
  while (strength === null) {
    const strengthInput = await showModal(
      'prompt',
      'Add Nation',
      `Enter Strength for "${trimmedName}":`,
      { defaultValue: '1', inputType: 'number' }
    );
    ...
    if (
      !isNaN(parsedStrength) &&
      Number.isInteger(parsedStrength) &&
      String(parsedStrength) === strengthInput.trim()
    ) {
      ...
      await showModal(
        'alert',
        'Invalid Input',
        'Strength must be a valid whole number (e.g., 0, 1, 5, -2).'
      );
...
export async function handleDeleteNation(event) {
  ...
  if (
    isNaN(indexToDelete) ||
    indexToDelete < 0 ||
    indexToDelete >= cfg.nations.length
  ) {
    console.warn(
      `Invalid index for deletion attempt. Index: ${indexToDelete}, Source: ${source}`
    );
    if (source === 'selection') {
  ...
  const nationName =
    cfg.nations[indexToDelete]?.name || `Nation at index ${indexToDelete}`;

  // Confirm deletion with the user
  const confirmDelete = await showModal(
    'confirm',
    'Delete Nation',
    `Are you sure you want to delete "${nationName}"? This cannot be undone.`,
    { confirmText: 'Delete Permanently', denyText: 'Cancel' }
  );
...
export async function saveInlineEdit() {
  ...
  if (editIndex === null || editIndex < 0 || editIndex >= cfg.nations.length) {
    console.warn(
      'Attempted to save inline edit with invalid index:',
      editIndex
    );
  ...
  if (
    isNaN(newStrength) ||
    !Number.isInteger(newStrength) ||
    String(newStrength) !== newStrengthStr
  ) {
    await showModal(
      'alert',
      'Validation Error',
  ...
  if (nameChanged && nation.flag) {
    // Only update if a flag name was already associated
    ...
    if (newFlagName && newFlagName !== oldFlagName) {
      ...
      console.log(
        `Nation name changed (index ${editIndex}). Updated flag base name from '${oldFlagName}' to '${newFlagName}'. Flag data/image remain associated.`
      );
      ...
      console.warn(
        `Could not generate a valid flag name for new nation name '${newName}'. Keeping old flag name '${oldFlagName}'.`
      );
    }
  }
