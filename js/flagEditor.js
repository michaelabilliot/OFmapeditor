...
let feOriginalPreview,
  feIntermediaryPanel,
  feIntermediaryPanelTitle,
  feIntermediaryPreview,
  feTransformedPreview;
...
export function loadImage(url) {
  return new Promise((resolve, reject) => {
    ...
    img.onerror = (err) => {
      console.error(
        `Flag Editor: Failed to load image: ${url ? url.substring(0, 100) : 'undefined'}...`,
        err
      );
...
export function initFlagEditor() {
  if (!mainCfg.flagEditorModalContainer) {
    console.error(
      'Flag Editor container element (#flagEditorModalContainer) not found in main HTML.'
    );
    return;
  }
  ...
  feCropModal?.addEventListener('click', (e) => {
    if (e.target === feCropModal) cancelFeManualCrop();
  });
...
export function openFlagEditor(
  originalFlagData,
  originalFlagType,
  originalWidth,
  originalHeight
  ...
  return new Promise(async (resolve) => {
    if (
      !mainCfg.flagEditorModalContainer ||
      !originalFlagData ||
      !originalFlagType
    ) {
    ...
    if (
      currentOriginalFlagType === 'svg' &&
      typeof currentOriginalFlagData === 'string'
    ) {
      ...
      console.error(
        'Flag Editor: Invalid source data - not SVG string or Data URL.'
      );
      flagEditorResolve = null;
      return resolve(null);
    }
...
async function handleFeApplyChanges() {
  ...
  if (feState.processingState !== 'done') {
    ...
    if (feState.processingState !== 'done') {
      alert(
        'Flag processing failed. Cannot apply changes. Check console for errors.'
      );
      return;
    }
  }
  ...
  if (feState.transformedUrl) {
    ...
    console.error(
      'Flag Editor: Processing done, but no transformed URL available.'
    );
...
async function displayFeCurrentImage() {
  ...
  if (feOriginalPreview) {
    ...
    feOriginalPreview.alt = `Original Flag`;
    feOriginalPreview.onerror = () => {
      feOriginalPreview.src = '';
      feOriginalPreview.alt = 'Error loading original';
    };
  }
  ...
  if (feState.processingState === 'idle') {
    ...
    try {
      ...
      alert(
        'An error occurred during flag processing. Check console for details.'
      );
    } finally {
      showFeLoading(false);
    ...
    if (
      ...
      feState.preprocessModeUsed === 'old_flags' &&
      feState.intermediaryUrl &&
      feState.intermediaryUrl !== feState.originalUrl
    ) {
      intermediaryTitle = 'Frame Removed';
      intermediarySrc = feState.intermediaryUrl;
    } else if (
      feState.transparencyCroppedUrl &&
      feState.transparencyCroppedUrl !== feState.originalUrl
    ) {
    ...
    if (
      ...
      feIntermediaryPreview.onerror = () => {
        feIntermediaryPreview.src = '';
        feIntermediaryPreview.alt = 'Error loading intermediary';
    ...
    if (feState.transformedUrl && feTransformedPreview) {
      ...
      feTransformedPreview.alt = `Standardized Flag`;
      feTransformedPreview.onerror = () => {
        feTransformedPreview.src = '';
        feTransformedPreview.alt = 'Error loading transformed';
      };
    }
...
function toggleFeManualCropButton() {
  ...
  if (showButton) {
    ...
    feDefineCropBtn.disabled =
      feState.processingState === 'processing' ||
      feState.processingState === 'error' ||
      !feState.originalUrl;
    // Update status text based on whether crop data exists
    feManualCropStatus.textContent = feState.manualCropData
      ? '(Crop Defined)'
      : '(No Crop Defined)';
  } else {
...
async function initFeCropper() {
  if (!feCropModal || !feCropImage || !window.Cropper) {
    // Check if Cropper lib is loaded
    console.error(
      'Flag Editor Cropper: Modal elements or Cropper.js library missing.'
    );
  ...
  try {
    ...
    if (feState.manualCropSourceUrl) {
      // If we have a previous manual crop result
      sourceForCropping = feState.manualCropSourceUrl;
      console.log('Using cached manual crop result for cropper.');
    } else if (feState.transparencyCroppedUrl) {
      // If we have a transparency crop result
      sourceForCropping = feState.transparencyCroppedUrl;
      console.log('Using cached transparency cropped URL for cropper.');
    } else {
      // Fallback to original (after ensuring transparency crop is attempted)
    ...
    if (!sourceForCropping) {
      throw new Error(
        'Could not prepare a valid image source for the cropper.'
      );
    }
    ...
    if (feCropperInstance) {
      try {
        ...
        console.warn('Error destroying previous cropper instance:', e);
      }
    ...
    feCropImage.onload = () => {
      ...
      if (!feCropImage.naturalWidth || !feCropImage.naturalHeight) {
        console.error(
          'Flag Editor Cropper: Image loaded but has zero dimensions.'
        );
        cancelFeManualCrop();
      ...
      try {
        feCropperInstance = new Cropper(feCropImage, {
          ...
          ready() {
            // Use ready event to apply previous crop data
            if (feState.manualCropData) {
              try {
                feCropperInstance.setData(feState.manualCropData);
              } catch (e) {
                console.warn(
                  'Could not apply previous crop data (might be invalid for current image size):',
                  e
                );
...
async function applyFeManualCrop() {
  ...
  try {
    ...
    if (!croppedCanvas)
      throw new Error('Failed to get cropped canvas from Cropper.');
...
async function cropTransparentArea(img) {
  ...
  try {
    ...
    console.warn(
      'Transparency Crop: Initial draw/getImageData failed. Retrying load...',
      e
    );
    try {
      const rImg = await loadImage(img.src); // Reload the image
      canvas.width = rImg.naturalWidth;
      canvas.height = rImg.naturalHeight;
      ctx.drawImage(rImg, 0, 0);
      imgData = ctx.getImageData(0, 0, canvas.width, canvas.height);
      img = rImg; // Update img ref
    } catch (rErr) {
      console.error('Transparency Crop: Image reload failed:', rErr);
      return img;
    } // Return original on reload failure
  }
  ...
  if (!croppedCtx) {
    console.error(
      'Transparency Crop: Could not get context for cropped canvas'
    );
    return img;
  }
  ...
  try {
    ...
    console.log(
      `Transparency Crop: Cropped from ${canvas.width}x${canvas.height} to ${croppedWidth}x${croppedHeight}`
    );
    return loadedCroppedImage;
  } catch (err) {
    console.error(
      'Transparency Crop: Failed to load cropped canvas data:',
      err
    );
    return img;
  } // Return original on final load error
}

async function removeUniformFrame(img) {
  // Constants for frame detection logic
  const MIN_DIMENSION = 20,
    CORNER_SAMPLE_SIZE = 15,
    OPAQUE_THRESHOLD = 200;
  const COLOR_TOLERANCE = 30,
    CONSISTENCY_MULTIPLIER = 2.5,
    CONSISTENCY_FLAT = 10;
  const MIN_FRAME_THICKNESS = 3;
  // Basic validation
  if (
    !img ||
  ...
  const ctx = canvas.getContext('2d', { willReadFrequently: true });
  if (!ctx) {
    console.error('Frame Removal: Could not get context');
    return img;
  }
  let imgData;
  ...
  try {
    ...
    console.warn(
      'Frame Removal: Initial draw/getImageData failed. Retrying load...',
      e
    );
    try {
      ...
      console.error('Frame Removal: Image reload failed:', rErr);
      return img;
    }
  }
  const data = imgData.data;
  ...
  const sampleLimitX = Math.min(CORNER_SAMPLE_SIZE, Math.floor(w / 2));
  const sampleLimitY = Math.min(CORNER_SAMPLE_SIZE, Math.floor(h / 2));
  for (let y = 0; y < sampleLimitY; y++) {
    for (let x = 0; x < sampleLimitX; x++) {
      const alphaIdx = (y * w + x) * 4 + 3;
      if (data[alphaIdx] > OPAQUE_THRESHOLD) {
        const rgbIdx = alphaIdx - 3;
        rSum += data[rgbIdx];
        gSum += data[rgbIdx + 1];
        bSum += data[rgbIdx + 2];
        count++;
  ...
  const isFramePixel = (x, y) => {
    ...
    const i = (y * w + x) * 4;
    if (data[i + 3] < OPAQUE_THRESHOLD / 2) return false;
    return (
      Math.abs(data[i] - frameColor[0]) <= COLOR_TOLERANCE &&
      Math.abs(data[i + 1] - frameColor[1]) <= COLOR_TOLERANCE &&
      Math.abs(data[i + 2] - frameColor[2]) <= COLOR_TOLERANCE
    );
  };
  // Detect frame thickness on all four sides
  let top = 0;
  while (top < h / 2 && isFramePixel(Math.floor(w / 2), top)) top++;
  let bottom = h - 1;
  while (bottom >= h / 2 && isFramePixel(Math.floor(w / 2), bottom)) bottom--;
  let left = 0;
  while (left < w / 2 && isFramePixel(left, Math.floor(h / 2))) left++;
  let right = w - 1;
  while (right >= w / 2 && isFramePixel(right, Math.floor(h / 2))) right--;
  const frameTop = top,
    frameBottom = h - 1 - bottom,
    frameLeft = left,
    frameRight = w - 1 - right;
  const minFrame = Math.min(frameTop, frameBottom, frameLeft, frameRight);
  const maxFrame = Math.max(frameTop, frameBottom, frameLeft, frameRight);
  // Check if frame thickness is consistent and meets minimum requirements
  if (
    minFrame < MIN_FRAME_THICKNESS ||
    maxFrame > minFrame * CONSISTENCY_MULTIPLIER + CONSISTENCY_FLAT
  ) {
    console.log(
      `Frame Removal: Skipping (Min: ${minFrame}, Max: ${maxFrame}, ReqMin: ${MIN_FRAME_THICKNESS})`
    );
    return img; // Frame inconsistent or too thin
  }
  // Calculate crop amount and new dimensions
  const cropAmount = minFrame;
  const newWidth = w - cropAmount * 2,
    newHeight = h - cropAmount * 2;
  if (newWidth <= 0 || newHeight <= 0 || (newWidth === w && newHeight === h))
    return img; // No change or invalid dimensions
  ...
  if (!cropCtx) {
    console.error('Frame Removal: Could not get context for crop canvas');
    return img;
  }
  cropCtx.drawImage(
    canvas,
  ...
  try {
    ...
    console.log(
      `Frame Removal: Removed frame of approx ${cropAmount}px. New size: ${newWidth}x${newHeight}`
    );
    return loadedCroppedImage;
  } catch (err) {
    console.error(
      'Frame Removal: Failed to load frame-removed canvas data:',
      err
    );
    return img;
  }
...
function feCreateRoundedRectPath(ctx, x, y, w, h, r) {
  if (w < 2 * r) r = w / 2;
  if (h < 2 * r) r = h / 2;
  ctx.beginPath();
  ctx.moveTo(x + r, y);
...
function feDrawRoundedImage(
  ...
  sx = 0,
  sy = 0,
  sw = img?.naturalWidth || dw,
  sh = img?.naturalHeight || dh
) {
  // Basic validation
  if (
    !img ||
    ...
    console.warn(
      'Flag Editor: DrawRoundedImage called with invalid image or dimensions.'
    );
 ...
 */
async function _performStandardizationPipeline(
  initialSourceImg,
  options,
  manualCropDetails = null
) {
  ...
  try {
    ...
    console.warn(
      'Transparency Crop step failed, continuing with previous source.',
      error
    );
  }
  ...
  if (
    options.preprocessMode === 'manual_crop' &&
    manualCropDetails?.sourceUrl
  ) {
    try {
      ...
      console.log(
        'Using manually cropped image as source for final transform.'
      );
    } catch (err) {
      console.error(
        `FE Pipeline: Failed to load manualCropSourceUrl. Error: ${err.message}. Falling back.`
      );
      // Fallback to the transparency-cropped version if manual crop fails to load
      sourceImg = await loadImage(
        transparencyCroppedUrl || initialSourceImg.src
      );
    ...
    try {
      ...
      if (frameRemovedImg.src !== sourceImg.src) {
        ...
        console.log(
          "'old_flags' mode selected, but no frame detected/removed."
        );
      }
    } catch (error) {
      console.warn(
        "'old_flags' frame removal step failed, continuing with previous source.",
        error
      );
    }
  } else {
    ...
    if (
      transparencyCroppedUrl &&
      transparencyCroppedUrl !== initialSourceImg.src
    ) {
      console.log(
        'Using transparency-cropped image as source for final transform.'
      );
    } else {
      console.log(
        'Using original (or failed transparency crop) image as source for final transform.'
      );
    }
  }
  ...
  if (innerFlagWidth <= 0 || innerFlagHeight <= 0)
    throw new Error(
      `Frame thickness (${options.frameThickness}px) too large for final dimensions.`
    );

  // Draw the background frame (e.g., black)
  ctx.fillStyle = '#000000'; // Frame color
  feCreateRoundedRectPath(
    ctx,
    0,
    0,
    options.finalWidth,
    options.finalHeight,
    options.fixedCornerRadius
  );
  ctx.fill();
  ...
  if (sourceWidth <= 0 || sourceHeight <= 0) {
    ...
    if (sw <= 0 || sh <= 0) {
      ...
      sy = 0;
      sw = sourceWidth;
      sh = sourceHeight; // Fallback to using full source
      console.warn(
        `FE Pipeline: Invalid crop calculation resulted in zero dimension. Using full source.`
      );
    }
    // Draw the cropped source into the rounded inner area
    // Adjust radius slightly for inner drawing
    let innerRadius = Math.max(
      0,
      options.fixedCornerRadius - options.frameThickness
    );
    feDrawRoundedImage(
      ctx,
      sourceImg,
  ...
  return {
    transformedUrl: finalTransformedUrl,
...
async function processImageTransformation() {
  ...
  try {
    ...
    console.error(
      `Flag Editor (Manual): Failed to load initial image ${feState.originalUrl}`,
      error
    );
    feState.processingState = 'error';
    throw error;
  ...
  const manualCropDetails =
    fePreprocessMode === 'manual_crop' &&
    feState.manualCropSourceUrl &&
    feState.manualCropData
      ? { sourceUrl: feState.manualCropSourceUrl, data: feState.manualCropData }
      : null;

  try {
    // Call the core pipeline function
    const result = await _performStandardizationPipeline(
      initialImage,
      options,
      manualCropDetails
    );
    ...
    if (
      result.intermediaryUrl &&
      result.intermediaryUrl !== feState.originalUrl &&
      feState.preprocessModeUsed !== 'manual_crop' &&
      feState.preprocessModeUsed !== 'old_flags'
    ) {
    ...
    console.error(
      `Flag Editor (Manual): CRITICAL ERROR during processing pipeline - ${error.message}`,
      error
    );
    feState.processingState = 'error';
 ...
 */
export async function standardizeFlag(
  sourceData,
  sourceType,
  sourceWidth,
  sourceHeight,
  ...
  if (
    ...
    !sourceData.startsWith('data:image/svg+xml')
  ) {
    initialImageUrl = svgStringToDataURL(sourceData); // Use exported function
    if (!initialImageUrl) {
      throw new Error(
        'Failed to convert SVG string to Data URL for standardization.'
      );
    }
  } else if (!String(initialImageUrl).startsWith('data:image')) {
    throw new Error(
      'Invalid sourceData provided for standardization: Must be SVG string or Data URL.'
    );
  }
  ...
  try {
    ...
    if (!initialImage.naturalWidth || !initialImage.naturalHeight) {
      ...
      console.warn(
        `StandardizeFlag: Loaded image has zero dimensions. Using provided ${sourceWidth}x${sourceHeight}`
      );
      // We can't directly set naturalWidth/Height, but pipeline should handle 0 dims
    }
  } catch (error) {
    console.error(
      `StandardizeFlag: Failed to load initial image from DataURL: ${initialImageUrl.substring(0, 100)}...`,
      error
    );
    throw new Error(
      `Failed to load flag image for standardization: ${error.message}`
    );
  }
  ...
  try {
    ...
    console.error(
      `StandardizeFlag: Error during processing pipeline - ${error.message}`,
      error
    );
